...
## Unity 取り込みレシピ（export --format fbx・P1-3）
...
## 主要な約束ごと（規約）
- **対象指定**: `--target <name>`（単数）または `--targets <name>`。既定は**完全一致のみ**（暗黙の正規表現フォールバックは無い）。正規表現で複数/曖昧一致させたいときは明示的に `--regex` を付ける（例: `bli select --targets "^Cube" --regex`）。完全一致 0 件のとき、それが正規表現として解釈すると当たる場合はエラーに `--regex` を使うヒントが付く。多くのコマンドは単一対象。
...
- **restricted は無確認で自走したいエージェント用途の推奨モード**: Blender API（`bpy`/`bmesh`/`mathutils` 等）は全面許可しつつ、プロセス起動・ネットワーク・削除系・動的実行（`eval`/`exec`等）・書込 `open` を AST で検出したら実行前に `EXEC_BLOCKED_RESTRICTED` で拒否する。ブロックされたら症状文の検出理由（例: `import:subprocess`）を見てコードを直せば再実行できる。有効化: `bli policy --action set --mode restricted`（人間が明示実行する）。
...
```
bli policy --action show --json                            # 現在の mode / policy.toml パスを確認
bli policy --action set --mode restricted --yes             # restricted へ昇格（人間が明示実行）
```
- サーバ（Blender アドオン）が読む真実源は常にユーザローカル `policy.toml`。このコマンドは表示/編集を助けるだけで **サーバへは何も送らない**（RPC なし・CLI フラグ単体で昇格できないという不変条件はそのまま）。
- `--action set` は既存の `policy.toml` に見覚えのない設定（`[exec]` 以外のセクション等）があると自動編集を拒否し、手動編集を促す（他の設定を壊さないため）。`--yes` を付けないと対話確認が入る。
