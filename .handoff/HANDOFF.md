...
> **全体俯瞰は `.handoff/ROADMAP.md`。v1（M0–M14）+ 設計レビュー P1 + P2-2 まで main マージ済み**（このファイルは全体史 + 規約・§6h=実地FB確定要約・§6i=M9・§6j=M10・§6k=M11・§6l=M12・§6m=M13・§6n=M14・§6o=設計レビュー P1・§6p=P2-2 Typer SSOT 自動生成・**§6q=P2-3 modifier/material 汎用化** 確定要約）。
...
- **検証**: pytest 794（+37: ops 検証 9 / fake-bpy 18〔test_gateway_modifier_props / test_gateway_material_extras〕/ 統合ロールバック 2〔test_ops_material_rollback＝fake gateway 注入〕/ behavior 8）/ 両版 OPS SMOKE OK（受け入れ基準: BEVEL/ARRAY を --props で追加・GLB に metallicFactor+images・FBX に PNG 埋込。**metallic の FBX バイナリ検証は Phong 系に標準フィールドが無く GLB のみ**）。
