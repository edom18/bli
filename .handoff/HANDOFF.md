...
最終更新: 2026-06-14 / 状態: **PR #1–#5（M0–M6 T6.3）マージ済み。M6 T6.4（modifier）は PR #6 で実装完了・セルフレビュー済み＝マージ待ち。PR #6 マージで M6 完了 → 次は M7（メッシュ編集）を `.handoff/NEXT-M7.md` 参照で着手**。
...
| **M6 汎用編集**（select/transform/apply-transform・duplicate/delete・material・modifier） | ✅ T6.1–6.3 main / T6.4 は PR #6 マージ待ち | pytest 151 + 5.0/4.4 実機 smoke OK |
| M7–M14 | 未着手（次=M7 メッシュ編集 / NEXT-M7.md） | — |

**状態（PR #6 ブランチ・M6 T6.4 まで）: `uv run pytest` = 151 passed / `ruff check` = 緑 / `ruff format --check` = 緑 / AST guard = OK / pyright は既存1件のみ（`bli/main.py` の narrowing・実行時安全）。PR #6 マージ前の main は 132 passed（T6.3 まで）。**
...
```
M6 は T6.1–6.4 完了（T6.4 は PR #6 マージ待ち）。**次は M7（メッシュ編集 bmesh一次）= `.handoff/NEXT-M7.md`**。
（PR #6 ブランチのベースライン緑は `uv run pytest` = 151 passed。M7 は PR #6 マージ後に main から新ブランチを切る＝共有ファイルのコンフリクト回避。）
...
- M6: T6.1–6.4 全完了（T6.4 modifier は PR #6 マージ待ち）。`exec-python` は M11。次は M7。
