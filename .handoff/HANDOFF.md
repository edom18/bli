...
最終更新: 2026-06-20 / 状態: **PR #1–#30 マージ済み（origin/main）。M0–M9 完了。M10 非同期job & フリーズ対策 ✅ 完了＝T10.1 job 化（#27）/ T10.2 render busy（#28）/ T10.3 watchdog（#30）。M9 確定要約は §6i・M10（T10.1/T10.2/T10.3）は §6j・GT research §E9–§E13。**次は M11 exec-python（着手書 `.handoff/NEXT-M11.md`・GUI スパイク不要）**。俯瞰 `.handoff/ROADMAP.md`**。

> 新規セッションはこの1枚を読めば再開できる。詳細は `specs/blender-cli-core/` を参照。
> **全体俯瞰は `.handoff/ROADMAP.md`。M10 は完了。次の作業は M11 exec-python＝着手書 `.handoff/NEXT-M11.md`**（このファイルは全体史 + 規約・§6h=実地FB確定要約・§6i=M9 確定要約・§6j=M10 確定要約）。
...
| **M11** exec-python（既定 off・audited/trusted 昇格・AST flag・監査） | ⬜ **次**（着手書 `.handoff/NEXT-M11.md`） | — |
| M12–M14 | 未着手 | — |

**状態（M10 完了＝PR #30）: `uv run pytest` = 351 passed / `ruff check` = 緑 / `ruff format --check` = 緑 / AST guard = OK / pyright（公式 = bli-core+bli-cli）= 0 errors（bli-addon は config で strict 対象外＝既存 `gateway.py` materials / `ops.py` name / `__init__.py` の ACCEPTED センチネル型は明示パス時のみ・実行時安全）。Blender 5.0.1/4.4.3 両版 background smoke OPS SMOKE OK（busy 拒否 + @persistent 生存 + watchdog 観測経路）+ GUI watchdog spike OK。**
...
- 経緯・スパイク・キックオフ判断の詳細は **`.handoff/NEXT-M10.md`（履歴化）**。次の作業は **M11 exec-python = `.handoff/NEXT-M11.md`**。
...
- M11: exec-python（既定 off・`EXEC_DISABLED` / audited / trusted 昇格・AST ヒューリスティック flag・監査記録）。着手書 `.handoff/NEXT-M11.md`。
