...
最終更新: 2026-07-12 / 状態: **PR #1–#47 マージ済み（origin/main=a0d8b7b）。v1（M0–M14）+ 設計レビュー P1 + P2-2（Typer SSOT ファクトリ）+ P2-3（modifier 任意 type + --props / material PBR・テクスチャ）完了＝pytest 794。確定要約は §6o（P1）・§6p（P2-2）・**§6q（P2-3）**・発注書は `report/2026-07-11-design-review-generality.md` §4（git 追跡済み）。次は P2-4（gateway/ops モジュール分割・純リファクタ）＝着手書 `.handoff/NEXT-P2.md`。俯瞰 `.handoff/ROADMAP.md`**。

> 新規セッションはこの1枚を読めば再開できる。詳細は `specs/blender-cli-core/` を参照。
> **全体俯瞰は `.handoff/ROADMAP.md`。v1（M0–M14）+ 設計レビュー P1 + P2-2 + P2-3 まで main マージ済み**（このファイルは全体史 + 規約・§6h=実地FB確定要約・§6i=M9・§6j=M10・§6k=M11・§6l=M12・§6m=M13・§6n=M14・§6o=設計レビュー P1・§6p=P2-2 Typer SSOT 自動生成・**§6q=P2-3 modifier/material 汎用化** 確定要約）。
> **現在の動き**: 実機デモ済み（zip → GUI 常駐 → bli のみでモデリング → capture / FBX export）。残る任意作業＝配布公開（PyPI/Extensions・CI artifact）。次の開発は **P2-4 モジュール分割**（`.handoff/NEXT-P2.md`・gateway 2,705 行 / ops 2,222 行が対象）。
...
| **M10** 非同期job & フリーズ対策（job 化 / render busy / watchdog） | ✅ **完了**（§6j）: T10.1 job 化（#27）/ T10.2 render busy（#28）/ T10.3 watchdog（#30） | pytest 351 + 5.0/4.4 実機 smoke OK + GUI スパイク OK |
...
| **M14** ドキュメント & 配布（addon zip ビルド・vendoring 検証・README・doctor 導入支援・mistakes-memo） | ✅ **完了**（§6n・PR #36/#37 マージ済み） | pytest 446 + 両版 vendored smoke OK |
| **設計レビュー P1 対応**（バグ4件 / exec restricted / add・mode・rename・parent・collection / FBX Unity opt / docs 再フレーミング / 検証型レビュー反映） | ✅ **完了**（§6o・PR #38/#43/#40/#41/#42 マージ済み） | pytest 584 + 両版 smoke OK + 実機デモ + review APPROVED |
| **P2-2 Typer SSOT 自動生成**（cli_factory / cli_specs / formatters・main.py 2,075→705 行・挙動不変） | ✅ **完了**（§6p・PR #45 マージ済み） | pytest 753 + surface/behavior スナップショット byte 一致（実機 smoke 対象外＝CLI のみ・wire は params 一致で担保） |
| **P2-3 modifier/material 汎用化**（--type 任意 + --props JSON / material PBR・テクスチャ・G4/G5） | ✅ **完了**（§6q・PR #47 マージ済み） | pytest 794 + 両版 smoke OK（--props golden 6 種・GLB metallicFactor/images・FBX PNG 埋込） |

**状態（v1 + 設計レビュー P1 + P2-2 + P2-3 まで main マージ済み・main=a0d8b7b）: `uv run pytest` = 794 passed / `ruff check` = 緑 / `ruff format --check` = 緑 / AST guard = OK / pyright = 0 errors。Blender 5.0.1/4.4.3 両版 background smoke OPS SMOKE OK（P2-3 golden 含む・最新実行 2026-07-12）+ 契約 CONTRACT OK + vendored smoke OK。`uv run` の Python は 3.12.10。コマンド数 37（cli-schema.json 実測）。**
...
## 6j. M10 非同期job & フリーズ対策（✅ 完了・T10.1/T10.2/T10.3）
M10 はサブPR分割（NEXT-M10.md・キックオフ D-A〜D-E 確定）。順序 T10.1 job 化 → T10.2 render busy → T10.3 watchdog（**全完了**）。**本質**: bpy はメインスレッド直列で重量ネイティブ処理は中断不能＝「非同期」は**接続を塞がない + 観測性**（メインの並列化ではない・spec §7 残存リスク）。土台は M4（settle/RUNNING/request-status）。**次は M11 exec-python = `.handoff/NEXT-M11.md`。**
...
### T10.2 render busy ✅（PR #28）— レンダ中は重量/破壊系を即拒否
...
### T10.3 watchdog ✅（PR #30）— メインスレッド応答性を検知し観測系に載せる【M10 完了】
- 重量ネイティブ処理がメインスレッドを占有して固まると pump タイマが止まる→これを**検知して観測可能にする**（**通知のみ＝実行は止めない／kill しない**・重量 op は中断不能）。spec §7 line 337。キックオフ確定（§4.5）: **R-C**=通知のみ / **R-D**=閾値 60s（=DISPATCH_TIMEOUT の2倍＝通常 op を誤検知しない安全側）/ 露出=request-status系+doctor。
- **仕組み**: pump tick（`dispatcher.install_timer` に `on_tick` フック追加）が生存印 `last_pump_ts` を更新→別スレッド監視（`watchdog._monitor_loop`）が「今−last_pump_ts>閾値」で unresponsive 判定＋`unresponsive_since` 記録＋初回ログ通知。`snapshot()` は**読み取り時にも**判定＝監視のポーリングに依存せず正しい（堅牢化）。受信スレッドが lock-free に snapshot を読み `request-status` 応答 `data.watchdog`（`{responsive, unresponsive_since, last_pump_age, threshold, kind=MAIN_THREAD_UNRESPONSIVE}`）へ載せる＝メインが固まっていても応答する。
- 実装: `watchdog.py`（新規・純Python・bpy 非依存＝pytest 可。生存印は単一 float＝GIL 下アトミックでロック不要、複合状態は `_lock`）/ `server.py` に `watchdog_status` 注入（既定 stub も同キー形・bpy 非依存のまま・`render_busy` と同流儀）/ `__init__.register` が `watchdog.start()`+`install_timer(on_tick=watchdog.mark_alive)`+`watchdog_status=watchdog.snapshot` を配線（unregister は timer 停止後に watchdog 停止）/ CLI は request-status・job-status human に注記 + `doctor` の `main_thread_responsive`（lock-free request-status 経由＝ping は固まると応答しないため別経路）+ **auto-wait/job-wait ポーリング中の固まりを一度だけ stderr 通知**。runtime に `WATCHDOG_UNRESPONSIVE_THRESHOLD=60`/`WATCHDOG_POLL_INTERVAL=5`。
- **GUI スパイク（`spikes/watchdog_spike.py`・GUI 必須・両版確定・研究 §E13）**: アイドル時 pump タイマが heartbeat 更新・実 decimate（ico subdivisions=7 ≈1.3M 面 apply・**5.0=0.247s/4.4=0.216s**）で timer 凍結（1.3M 面でも 1s 未満→**60s 閾値は通常 op を誤検知しない安全側**）・占有（sleep>閾値）中に**別スレッドが unresponsive を観測**（両版 90 サンプル）＝request-status 観測性の裏付け・回復確認。**GIL 注記**: 実 native op が GIL を握り続ける場合は別スレッド観測が占有解除後に成立し得る（凍結検知自体は成立・v1 許容＝spec §7 残存リスク）。
- **セルフレビュー（独立3視点）で解消**: **P1** auto-wait/job-wait のポーリングが watchdog を握り潰す（既定経路で固まりが不可視）→ 一度だけ stderr 通知。**P2** 通知が実閾値を表示 / 既定 stub のキー形を snapshot と一致 / `MAIN_THREAD_UNRESPONSIVE` を kind として露出。**P3** unregister 順序（timer 停止後に watchdog 停止）/ doctor の例外握りを Exception に広げ / 回復通知に凍結時間。pytest 351・両版 OPS SMOKE OK + GUI watchdog spike OK。
- **繰越**: `print-export` の `is_heavy=False` だが methods.md 表 `H=✓`＝ドリフト要再整合（M10 全体の繰越・T10.2 から継続）。output_ref GC（M5 繰越）/ `--dry-run` 一般化（M13）。
- 経緯・スパイク・キックオフ判断の詳細は **`.handoff/NEXT-M10.md`（履歴化）**。

## 6k. M11 exec-python（✅ 完了・T11.1/T11.2/T11.3・PR #32）
exec-python = 構造化サブコマンドで表現できない操作の逃げ道（spec D3・既定 off・**サンドボックスなし**＝防止でなく検知 §459）。GUI スパイク不要（既存 Dispatcher のメインスレッド直列で実 bpy・研究 §E14）。キックオフ R-A〜R-E を全て推奨案でユーザー確定。**R-A が肝**: exec mode/allow_hashes の真実源は **サーバが読むユーザローカル `policy.toml`**（`BLI_STATE_DIR/policy.toml`）のみ＝CLI/params/`.bli/config.toml` から昇格できない。各サブタスクで独立3視点セルフレビュー（設計/セキュリティ敵対的/仕様）＝**P1 ゼロ**。
- **T11.1 mode ゲート + 最小実行**: `policy.py`（tomllib・fail-closed＝不在/不正/非table/不正 mode は全て off・実行ごとに最新読取）/ `exec_runner.py`（純Python・bpy 非依存・ast 分割で最終式 repr〔**None は displayhook 流儀で抑制**〕・stdout/stderr キャプチャ・実行段は **`except BaseException`**＝ユーザコードの `sys.exit()` が dispatch を巻き込まない）/ `gateway.exec_user_code`（bpy 注入境界 `{"bpy":bpy}`・新規 namespace 毎回）/ `ops._exec_python`（mode!=trusted→`EXEC_DISABLED`・code/file 排他を bpy 前検証・例外→`EXEC_ERROR`〔runtime=ENVIRONMENT/compile=USER_INPUT・例外直前 stdout/stderr を cause に〕・`_ok_offload` 退避）/ CLI `exec-python --code|--file`（**--file は CLI 側 CWD で読み code 送信**＝Blender CWD と区別・サーバ側 file 読取は直接 RPC 用フォールバックで存在確認のみ・**配下封じ込めは意図的に課さない**＝trusted 前提では open() で任意読めるため security 価値なし）。exec-python を `implemented=True` 化（発見系露出・schema_hash 更新・test_cli_help は合成未実装コマンド `fake-unimpl-cmd` でフィルタ機構検証に変更）。errors に `EXEC_ERROR`。セルフレビュー反映: BaseException 捕捉（SystemExit 漏れ）/ spec §289/§385/§470/§493 の policy 配置ドリフト修正 / §E14 None 抑制注記 / file 経路テスト追加。
- **T11.2 AST ヒューリスティック flag（R-D）**: `ast_heuristics.py`（純Python）。注目モジュール import（os/subprocess/socket/shutil/urllib/ctypes/pickle/...＝top で集約）/ eval・exec・compile・__import__・breakpoint / 書き込み open（'w'/'a'/'x'/'+'・非定数 mode は保守的）を `heuristic_flags` に列挙。**ブロックしない**（実行可否は mode ゲートのみ・mode と独立）。`scan` は **決して例外を投げない**契約（構文エラー/null byte→[]）。セルフレビュー反映: ValueError 捕捉 / file-write は組込み open のみ対象と明示 / breakpoint 追加。
- **T11.3 監査 + 許可ハッシュ（R-B・spec §280）**: `audit.py`（`BLI_STATE_DIR/audit/exec.jsonl` に JSONL 追記・**生コードは書かず sha256 のみ**＝インジェクション耐性・executed も rejected:off/audited-unlisted も記録・best-effort で失敗時 `audit_ok=false`+stderr 警告）/ `policy.read_allow_hashes`（strip+lower 正規化・fail-closed）/ `runtime.audit_dir`。**audited（R-B）**: コードの sha256 が `[exec] allow_hashes` に一致したときだけ自走実行・不一致は記録して拒否し追加すべき sha を提示。trusted は無条件。応答に `code_sha256`・`audit_ok` 追加。CLI human は flags と audit 失敗を表示。セルフレビュー反映: rejection 経路の監査失敗を observable 化 / allow_hashes 正規化 / read_entries の壊れ行 skip / 大小無視の統合テスト。**敵対的レビュー結論: allow-hash ゲート・fail-closed・R-A・TOCTOU 不在（source を一度読んで sha も実行も同一）・JSONL 安全をすべて確認・P1 なし**。
- **検証**: pytest 433（T11.1〜T11.3 で L1 多数・test_policy/exec_runner/exec_ops/ast_heuristics/audit）+ 両版 background smoke の exec ブロック（off拒否→audited(unlisted 拒否/listed 自走)→trusted→実 bpy mutate→EXEC_ERROR→off 戻し再拒否 + AST flag + 監査ログ3経路）+ exec_spike 両版 OK。
- **繰越（M14 以降・単一ユーザ前提で許容）**: audited の監査失敗を fail-closed にするか（v1 は best-effort）/ policy.toml の所有者・権限検証（POSIX）/ BLI_STATE_DIR env で policy 真実源が差し替わる（同一 OS ユーザに帰着・§459）/ exec は is_heavy=False＝job 化せずメイン同期占有し得る（M10 watchdog 通知対象）。

## 6l. M12 Skill 同梱 & スキーマ同期（✅ 完了・PR #33・stacked on #32）
AIエージェントが bli を発見・利用できるよう Claude Code Skill を同梱し、コマンドカタログを SSOT から生成してドリフトを検出する（D12）。
- **T12.1** `.claude/skills/bli/SKILL.md`: frontmatter（name: bli / description）+ 接続（doctor/ping）+ コマンド発見（list-commands/help/cli-schema.json）+ 3シナリオ定石（set-origin/straighten/print-*・**実 CLI フラグと突き合わせ済み**）+ 規約（targets/output_ref+--fetch/終了コード/request-status・job-wait/--id 冪等）+ exec-python（既定 off・policy.toml 昇格・サンドボックスなし）。
- **T12.2** `scripts/generate_cli_schema.py` + `.claude/skills/bli/reference/cli-schema.json`: 全コマンド（31）のメタ + JSON Schema + schema_hash を SSOT から **決定的**に生成（sort_keys・接続不要）。SSOT を変えたら再生成（`uv run python scripts/generate_cli_schema.py`）。
- **T12.3** `packages/bli-cli/tests/test_skill_schema_sync.py`: キャッシュの schema_hash == ライブ SSOT を強制（**schema snapshot・ドリフトで fail**）+ 再生成完全一致 + 実装済み網羅 + frontmatter。smoke にも hello（アドオンの schema_hash）== cli-schema.json の一致チェック追加（`skill_schema_sync_ok`）。

## 6m. M13 テスト網羅 & CI 仕上げ（✅ 完了・PR #34・stacked on #33）
全層 CI 緑・5.0 必須ゲートを満たす。
- **T13.1 CI L2 マトリクス**: `.github/workflows/test-blender.yml` の `bpy-integration` を有効化。setup-blender で 5.0/4.4（**fail-fast:false**）、golden smoke（grep `OPS SMOKE OK`）+ 契約テストを実機実行。**5.0=必須ゲート / 4.4=continue-on-error（ベストエフォート）**。L1 job に pyright 追加。**注**: setup-blender@v5 が Blender 5.0 を解決できない場合は CI 側で version 調整（実装ロジックは正）。
...
## 6o. 設計レビュー P1 対応（✅ 完了・2026-07-11・PR #38/#43/#40/#41/#42 マージ済み）
発注書＝**`report/2026-07-11-design-review-generality.md`**（uLoop 比較で「3シナリオ過適合」を確認した全体設計レビュー）。§4 の P1-0〜P1-3 + P2-1 を実施し、仕上げに `spec-workflow:review-orchestrator`（find→verify→fix→gate の検証型レビュー）で **APPROVED** まで収束。
- **P1-0 バグ4件（#38）**: B1=exit code を `ErrorObject.retryable` 参照ベース化（SESSION_BUSY/IN_PROGRESS→2・AUTH_FAILED/PROTOCOL_VERSION_MISMATCH→3）/ B2=**resolve_targets の暗黙 regex フォールバック廃止**（完全一致既定・targets を持つ全コマンドに `--regex` BOOL 新設・0件時「--regex なら N 件」ヒント）/ B3=object_summary を list_object_materials（slot.link 尊重）経由へ / B4=save_blend `except Exception`→E_OPERATOR。
- **P1-1 exec restricted（#43=旧#39 再作成）**: policy mode=off|**restricted**|audited|trusted。`exec_restricted.scan_blocked`＝AST ブロックリスト（bpy/bmesh/mathutils 全面許可・プロセス/ネットワーク/削除系/動的実行/書込 open を `EXEC_BLOCKED_RESTRICTED` で拒否・別名/from-import 追跡）。監査 `blocked` フィールド（None=対象外/[]=通過/非空=理由）。**policy 読み書きは `bli_core/policy.py` に集約**（addon 側は再エクスポート）。CLI `bli policy --action show|set`（RPC なし・set は人間実行前提＝spec §6 に信頼境界明記）。R-A 原則（policy.toml 真実源・CLI 昇格不可・fail-closed）不変。
- **P1-2 プリミティブ5コマンド（#40）**: `add`（11種・前後名前差分で新規特定）/ `mode`（E_MODE_MISMATCH の remediation を「bli mode --to object」へ）/ `rename`（衝突実名報告）/ `parent`（循環検出・**keep_transform は world 退避→復元**）/ `collection`（unlink 所属0ガード）。
- **P1-3 FBX Unity オプション（#41）**: `--axis-forward/--axis-up/--scale/--apply-unit-scale/--embed-textures`（fbx 専用 presence-sensitive・両版で export_scene.fbx パラメータ完全同一を実機確定・rna 検査で silent drop 禁止・embed→path_mode=COPY 自動）。負軸値は `=` 連結（SKILL 記載）。
- **P2-1 docs 再フレーミング（#42）**: SKILL.md を「汎用ワークフロー（発見→観察→編集→検証→入出力）」中心へ・3シナリオは「代表レシピ」へ降格（全文維持）・spec §1 を汎用基盤へ再定義。
- **検証型レビュー反映（#42 内 7b9bb28）**: 確定指摘7件を修正＝parent_set 実バグ（実機 delta=10→0）/ exec 昇格誘導の遮断 / policy 書込集約 / **list-objects の名前フィルタを `--name-regex` へ改名**（targets 系 BOOL `--regex` と同名で silent 失敗・旧名は CLI 別名受理・`--`始まり値は loud 拒否）/ docs 4モード一括整合 / **bli-core の tomllib 3.10 ガード**（try import + fail-closed 縮退・実 3.10.6 検証）。偽陽性0・suggestions 10 件は `.claude/review/state.json`（git 非管理）に記録。
- **落とし穴（今回学び）**: ①リポジトリの「head ブランチ自動削除」設定でスタック PR の base が消えると子 PR が自動 CLOSE される→**スタックのマージは「子 PR を先に main へリターゲット→マージ」の順**（#39 が犠牲→#43 で再作成）。②常駐 GUI Blender（デモ）が 9876 を占有すると E2E テストが誤爆→テスト実行前に常駐を落とす。③fake-bpy テストは `_forget_gateway_module()`（sys.modules + 親パッケージ属性の両方を消す）を踏襲（mistakes-memo 記載）。
- **実機デモ**: `dist/launch_dist.py` で zip 常駐 → bli のみで床/テーブル（天板+脚4本・親子/コレクション）/樽をモデリング → capture / print-check / FBX export。シーンは repo 直下 `demo_table_scene.blend`（未追跡）。
- **検証**: pytest 584 / ruff・format・pyright・AST guard 緑 / 両版 smoke OK / gate 3ラウンド pass。

## 6p. P2-2 Typer コマンドの SSOT 自動生成（✅ 完了・2026-07-12・PR #45 マージ済み）
発注書＝report §4 P2-2。`models.model_for()`（SSOT→Pydantic 動的生成）と同じパターンを Typer へ。**挙動を変えない内部リファクタ**（definitions.py 無変更＝schema_hash 不変）。
- **成果**: 新コマンド追加の必須変更が 7–8 箇所 → **definitions.py + ops.py + gateway.py（+任意で formatters.py の HUMAN_FORMATTERS 登録）+ 再生成**（`scripts/generate_cli_schema.py` と `packages/bli-cli/tests/regen_snapshots.py`＝新コマンドは surface スナップショットに載るため regen 必須）。main.py **2,075→705 行**。
- **構成**（bli-cli）: `cli_factory.py`＝Param（型/required/default/choices/help）→ typer.Option 写像・動的 `__signature__` コールバック・infra オプション（--fetch/--id/--async/--json/--port）自動付与（--id は mutates or is_heavy が既定則）/ `cli_specs.py`＝CLI 固有互換のオーバーライド表（docstring 32 件・help 文言差 74 件・`--target` 別名は全コマンド共通既定則・always_send/tristate/cli_defaults・pre_hook/build hooks。**新コマンドはエントリ不要**）/ `formatters.py`＝HUMAN_FORMATTERS（未登録は JSON フォールバック）。手書き維持=ping/doctor/init/policy/job-wait/list-commands/help（hello 由来 payload・RPC なしローカル・`_rpc` 非経由のため）。
- **送信ポリシー既定則**（presence-sensitive の一般化）: required=常時送信 / BOOL default=True=`--x/--no-x` 常時送信 / BOOL それ以外=フラグ・True のみ送信 / 非BOOL default あり=常時送信 / なし=指定時のみ。**歴史的逸脱は specs で明示**: always_send=`use_selection`（export）・`ascii`（print-export）・`force`（open）/ tristate=`apply_unit_scale` / cli_defaults=print-export `format=stl`（SSOT は required）。
- **移行ガード（受け入れ基準の実装）**: 先にスナップショット固定 → 移行 → byte 一致検証。surface=`tests/snapshots/cli_surface.json`（39 コマンドの click 構造＝オプション名/別名/required/default/型/help 文言。rich 描画は端末幅依存のため構造を真実源に）/ behavior=`cli_behavior.json`（82 ケース×human/json・fake client+決定的 UUID・exit code/stdout/stderr/**送信 (method, params)**）。再生成=`uv run python packages/bli-cli/tests/regen_snapshots.py`（diff は必ずレビュー）。
- **付随**: `packages/bli-core/tests/test_methods_md_sync.py`＝contracts/methods.md のドリフト検出（生成せず構造照合: コマンド網羅 / 表の逆方向 / 全 param の `--kebab` 表記・`with_object`→`--with`）。現状ドリフト 0。
- **落とし穴（今回学び）**: ①request-status/job-status の `--id` は**ドメイン param**（RPC request_id と別物）＝factory は has_id で分岐して取り違え防止 ②typer 0.26 は click を `typer._click` にベンダリング（venv に click 単体は無い） ③動的シグネチャの annotation は**実型オブジェクト必須**（文字列 `"float | None"` は Typer が解決できない） ④CLI と SSOT の help 文言差の SSOT 側統一は schema_hash が変わるため見送り（フォローアップ候補・ユーザー判断）。
- **検証**: pytest 753（584+スナップショット 166+methods.md 同期 3）/ ruff・format 緑 / 実プロセス `--help` 描画・コマンド一覧順維持を目視確認。実機 smoke 未実施（CLI のみ・wire 互換は params byte 一致で担保）。

## 6q. P2-3 modifier/material 汎用化（✅ 完了・2026-07-12・PR #47 マージ済み）
発注書＝report §4 P2-3（P2 で唯一の機能追加・G4/G5）。着手前スパイクで **5.0.1/4.4.3 両版の完全一致**（modifier rna 83 種・Principled 入力名・Image Texture 接続・rna は範囲外を clamp / float32 格納＝比較は 1e-6 許容）を確定＝**版差分岐ゼロ**。
- **modifier（G4）**: `--type` を ENUM(5種)→STR 化し `gateway.require_modifier_type`（`bpy.types.Modifier` の rna enum から**能力検出**・無効は有効一覧つき INVALID_PARAMS）。`--props '<JSON>'`＝`gateway.set_modifier_props`/`_coerce_prop_value`（編集可能 rna プロパティを列挙し型検証: BOOLEAN/INT/FLOAT〔配列〕/ENUM/STRING/POINTER〔Object 名前解決のみ・他は明示拒否〕。未知キーは有効一覧提示・全キー検証後に setattr）。**範囲外は rna が clamp → `applied_props` に設定後実値を返して可視化**。専用フラグと併用不可。**BOOLEAN は props.object 必須 + `--with` と同一の `_resolve_boolean_operand` 検証**（自己参照禁止・mesh 限定＝検証二重定義ドリフト防止）。検証失敗時は modifier をアトミック撤去。
- **material（G5・create 専用）**: `--metallic/--roughness/--alpha`（0..1 を bpy 前検証）/`--emission`+`--emission-strength`（省略時 **1.0 明示**＝silent 無効化防止）/`--texture`（Image Texture→Base Color 接続・パス不在は bpy 前 USER_INPUT・壊れ画像/pack 失敗は E_OPERATOR）/`--pack-texture`。Principled 入力欠如は E_PRECONDITION（silent drop なし）。**アトミック性**: 失敗時は material+image を撤去（`discard_created_material`・捕捉は **BaseException 統一＋必ず再送出**）。**共有 mesh ガード（不可逆な単一ユーザ化）は create_material 成功後**＝失敗時に mesh を分離しない（レビュー R2-A）。
- **CLI は P2-2 ファクトリで手書きゼロ**（SSOT 追加のみ→schema/スナップショット再生成。cli_specs の help_overrides は 74→73 件へ 1 件減＝modifier type の旧 override 除去）。
- **レビュー（PR ゲート・review-orchestrator）**: 4 finder × 3 ラウンド + **ユーザー承認の追加 1 ラウンド**（max 到達時に小粒 2 件が残り ESCALATED→承認延長）＝APPROVED。修正 11 / 偽陽性棄却 1（--texture の UNC パス＝import/export と同型の既存脅威モデル内）/ suggestions 4（--props の FILE_PATH subtype・テクスチャサイズ上限ほか＝次の品質パス候補）。
- **インシデント（解決済み・mistakes-memo 記録）**: smoke の clamp golden が BEVEL segments=1000 を残置→後続 modifier 追加で depsgraph 評価が走りジオメトリ爆発（一時 blender.exe がメモリ大量消費・ユーザー発見）。**重い評価を誘発し得る modifier は assert 後即 remove** が対策。
- **検証**: pytest 794（+41: ops 検証 13 / fake-bpy 18〔test_gateway_modifier_props / test_gateway_material_extras〕/ 統合ロールバック 2〔test_ops_material_rollback＝fake gateway 注入〕/ behavior 8）/ 両版 OPS SMOKE OK（受け入れ基準: BEVEL/ARRAY を --props で追加・GLB に metallicFactor+images・FBX に PNG 埋込。**metallic の FBX バイナリ検証は Phong 系に標準フィールドが無く GLB のみ**）。
...
- M10: **完了**（T10.1 job 化 #27 / T10.2 render busy #28 / T10.3 watchdog #30・確定要約 §6j・GT research §E12/§E13）。M10 繰越: `print-export` の `is_heavy=False` vs methods.md `H=✓` ドリフト / output_ref GC（M5 繰越）/ `--dry-run` 一般化は M13。**次は M11**。
...
- M14: **完了**（addon zip ビルド `scripts/build_addon.py` + vendoring 隔離テスト + 両版 vendored smoke + doctor 導入支援 + README + mistakes-memo・確定要約 §6n・PR #36/#37 マージ済み）。M14 繰越: Extensions 形式配布（`blender_manifest.toml` は予約済み）/ PyPI 公開で pipx 対応 / CI artifact 化。GUI 実機導入は §6o のデモで実質確認済み（Install from Disk の literal 手順のみ未実施）。
- 設計レビュー P1 対応: **完了**（§6o・PR #38/#43/#40/#41/#42）。繰越＝P2（着手書 `.handoff/NEXT-P2.md`・発注書 report §4）+ レビュー suggestions 10 件（`.claude/review/state.json`・git 非管理: regex 時間制限 / _OS_DELETE_ATTRS 拡充 / parent・collection 空 targets ガード / regex param 定義の 19 回コピー / ABORTED kind 未登録 等）。P3 候補: logs コマンド / new（空シーン）/ batch 実行 / audited 運用改善（report §4 P3）。
- P2-2 Typer SSOT 自動生成: **完了**（§6p・PR #45）。フォローアップ候補: cli_specs の help 文言差（実測 73 件・P2-3 で 1 件減）を SSOT 側へ統一（schema_hash が変わる＝ユーザー判断）。G4（regex param 定義の 19 回コピー）は SSOT 側の話として残存（cli 側は factory で解消）。
- P2-3 modifier/material 汎用化: **完了**（§6q・PR #47）。繰越＝**P2-4 モジュール分割（次・gateway 2,705 行 / ops 2,222 行・main.py 705 行は対象外）** + レビュー suggestions（--props の STRING FILE_PATH subtype パス検証 / テクスチャ読込のサイズ上限・MemoryError 捕捉＝いずれもプロジェクト全体のパスポリシー議論・P3 候補）。
