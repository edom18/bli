...
最終更新: 2026-07-12 / 状態: **PR #1–#45 マージ済み（origin/main=5dd3fac）。v1（M0–M14）+ 設計レビュー P1 + P2-2（Typer SSOT 自動生成＝挙動不変リファクタ）完了＝pytest 753。確定要約は §6o（P1）・**§6p（P2-2）**・発注書は `report/2026-07-11-design-review-generality.md` §4。次は P2-3（modifier/material 汎用化＝機能追加・要 rna スパイク）→ P2-4（モジュール分割）＝着手書 `.handoff/NEXT-P2.md`。俯瞰 `.handoff/ROADMAP.md`**。

> 新規セッションはこの1枚を読めば再開できる。詳細は `specs/blender-cli-core/` を参照。
> **全体俯瞰は `.handoff/ROADMAP.md`。v1（M0–M14）+ 設計レビュー P1 + P2-2 まで main マージ済み**（このファイルは全体史 + 規約・§6h=実地FB確定要約・§6i=M9・§6j=M10・§6k=M11・§6l=M12・§6m=M13・§6n=M14・§6o=設計レビュー P1・**§6p=P2-2 Typer SSOT 自動生成** 確定要約）。
> **現在の動き**: 実機デモ済み（zip → GUI 常駐 → bli のみでモデリング → capture / FBX export）。残る任意作業＝配布公開（PyPI/Extensions・CI artifact）。次の開発は **P2-3 modifier/material 汎用化 → P2-4 モジュール分割**（`.handoff/NEXT-P2.md`）。
...
**状態（v1 + 設計レビュー P1 + P2-2 まで main マージ済み・main=5dd3fac）: `uv run pytest` = 753 passed / `ruff check` = 緑 / `ruff format --check` = 緑 / AST guard = OK / pyright = 0 errors。Blender 5.0.1/4.4.3 両版 background smoke OPS SMOKE OK（parent 再親付け回帰含む）+ 契約 CONTRACT OK + vendored smoke OK（実機 smoke は P1 時点・P2-2 は CLI のみで対象外）。`uv run` の Python は 3.12.10。コマンド数 37（cli-schema.json 実測・旧記載 38 は誤記）。**
...
- P2-2 Typer SSOT 自動生成: **完了**（§6p・PR #45）。繰越＝**P2-3 modifier・material 汎用化（次・要 rna スパイク）/ P2-4 モジュール分割（gateway/ops が残り・main.py は P2-2 で縮小済み）**。フォローアップ候補: cli_specs の help 文言差（実測 74 件）を SSOT 側へ統一（schema_hash が変わる＝ユーザー判断）。G4（regex param 定義の 19 回コピー）は SSOT 側の話として残存（cli 側は factory で解消）。
