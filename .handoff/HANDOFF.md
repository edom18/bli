...
| **P2-2 Typer SSOT 自動生成**（cli_factory / cli_specs / formatters・main.py 2,076→705 行・挙動不変） | ✅ **完了**（§6p・PR #45 マージ済み） | pytest 753 + surface/behavior スナップショット byte 一致（実機 smoke 対象外＝CLI のみ・wire は params 一致で担保） |

**状態（v1 + 設計レビュー P1 対応まで main マージ済み・main=c5d5544）: `uv run pytest` = 584 passed / `ruff check` = 緑 / `ruff format --check` = 緑 / AST guard = OK / pyright = 0 errors。Blender 5.0.1/4.4.3 両版 background smoke OPS SMOKE OK（parent 再親付け回帰含む）+ 契約 CONTRACT OK + vendored smoke OK。`uv run` の Python は 3.12.10。コマンド数 38（cli-schema.json）。**
...
- **成果**: 新コマンド追加の必須変更が 7–8 箇所 → **definitions.py + ops.py + gateway.py（+任意で formatters.py の HUMAN_FORMATTERS 登録）+ 再生成**（`scripts/generate_cli_schema.py` と `packages/bli-cli/tests/regen_snapshots.py`＝新コマンドは surface スナップショットに載るため regen 必須）。main.py **2,076→705 行**。
- **構成**（bli-cli）: `cli_factory.py`＝Param（型/required/default/choices/help）→ typer.Option 写像・動的 `__signature__` コールバック・infra オプション（--fetch/--id/--async/--json/--port）自動付与（--id は mutates or is_heavy が既定則）/ `cli_specs.py`＝CLI 固有互換のオーバーライド表（docstring・help 文言差 約90件・`--target` 別名は全コマンド共通既定則・always_send/tristate/cli_defaults・pre_hook/build hooks。**新コマンドはエントリ不要**）/ `formatters.py`＝HUMAN_FORMATTERS（未登録は JSON フォールバック）。手書き維持=ping/doctor/init/policy/job-wait/list-commands/help（hello 由来 payload・RPC なしローカル・`_rpc` 非経由のため）。
...
- P2-2 Typer SSOT 自動生成: **完了**（§6p・PR #45）。繰越＝**P2-3 modifier・material 汎用化（次・要 rna スパイク）/ P2-4 モジュール分割（gateway/ops が残り・main.py は P2-2 で縮小済み）**。フォローアップ候補: cli_specs の help 文言差（約90件）を SSOT 側へ統一（schema_hash が変わる＝ユーザー判断）。G4（regex param 定義の 19 回コピー）は SSOT 側の話として残存（cli 側は factory で解消）。
