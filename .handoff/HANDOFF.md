...
最終更新: 2026-07-11 / 状態: **PR #1–#43 マージ済み（origin/main=c5d5544）。v1（M0–M14）完了 + 設計レビュー対応 P1（P1-0〜P1-3 + P2-1 + 検証型レビュー反映）完了＝コマンド 33→38・pytest 584。確定要約は §6o・発注書は `report/2026-07-11-design-review-generality.md` §4。次は P2-2（Typer SSOT 自動生成）→ P2-3（modifier/material 汎用化）→ P2-4（モジュール分割）＝着手書 `.handoff/NEXT-P2.md`。俯瞰 `.handoff/ROADMAP.md`**。

> 新規セッションはこの1枚を読めば再開できる。詳細は `specs/blender-cli-core/` を参照。
> **全体俯瞰は `.handoff/ROADMAP.md`。v1（M0–M14）+ 設計レビュー P1 対応まで main マージ済み**（このファイルは全体史 + 規約・§6h=実地FB確定要約・§6i=M9・§6j=M10・§6k=M11・§6l=M12・§6m=M13・§6n=M14・**§6o=設計レビュー P1 対応** 確定要約）。
> **現在の動き**: 実機デモ済み（zip → GUI 常駐 → bli のみでモデリング → capture / FBX export）。残る任意作業＝配布公開（PyPI/Extensions・CI artifact）。次の開発は **P2 リファクタ&汎用化**（`.handoff/NEXT-P2.md`）。
...
- 設計レビュー P1 対応: **完了**（§6o・PR #38/#43/#40/#41/#42）。繰越＝**P2-2 Typer SSOT 自動生成 / P2-3 modifier・material 汎用化 / P2-4 モジュール分割**（着手書 `.handoff/NEXT-P2.md`・発注書 report §4）+ レビュー suggestions 10 件（`.claude/review/state.json`・git 非管理: regex 時間制限 / _OS_DELETE_ATTRS 拡充 / parent・collection 空 targets ガード / regex param 定義の 19 回コピー / ABORTED kind 未登録 等）。P3 候補: logs コマンド / new（空シーン）/ batch 実行 / audited 運用改善（report §4 P3）。
