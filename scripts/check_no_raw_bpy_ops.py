...
- ラッパ定義ディレクトリ（既定 `bli_addon/gateway`＝gateway.py をパッケージへ分割した配下）。
  `--allow-dir` で追加可。裸のディレクトリ名ではなく**アンカー付きセグメント列**
  （パス中に `bli_addon/gateway` が連続して現れる場合のみ一致）で判定する。
  別位置の同名ディレクトリ（例: `ops/gateway/`）を誤って免除しないため。
- `spikes/` 配下（実験コード）。
...
def _matches_allow_dir(parts: tuple[str, ...], allow_dirs: set[str]) -> bool:
    """パス構成要素に許可ディレクトリのセグメント列が**連続部分列**として含まれるか。

    裸名の集合演算（`set(parts) & allow_dirs`）だと任意階層の同名ディレクトリを
    丸ごと免除してしまうため、`bli_addon/gateway` のようにアンカーして判定する。
    """
...
def check(paths: list[str], allow: set[str], allow_dirs: set[str] | None = None) -> list[str]:
    ...
    for f in _iter_py_files(paths):
        ...
        if f.name in allow:
            continue
        if _matches_allow_dir(f.parts, allow_dirs):
            continue
        try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        action="append",
        default=[],
        help="許可するディレクトリ（'bli_addon/gateway' 形式のアンカー付きセグメント列・追加）",
    )
