...
検証（golden）:
  ...
  → mesh (M7 T7.2)                  : extrude（8→16v）/ bevel（seg1 → 24v）/ inset（→32v）/
                                       op別必須・範囲ガード
  → mesh (M7 T7.3)                  : boolean（modifier 経由・INTERSECT/DIFFERENCE の world bbox）/
                                       decimate（ico 80f→40f）/ 相手 自己参照・非mesh ガード
"""
...
def ensure_mesh_fixtures():
    ...
    # MeshBoolA/MeshBoolC を MeshBoolB（x+1 平行移動）に対して INTERSECT/DIFFERENCE する
    # （operand B は read-only で2回再利用・world bbox が幾何的に決定的＝solver 非依存の golden）。
    ...
    boola = _fresh_cube_obj("MeshBoolA")
    boola.location = (0.0, 0.0, 0.0)
    boolc = _fresh_cube_obj("MeshBoolC")
    boolc.location = (0.0, 0.0, 0.0)
    boolb = _fresh_cube_obj("MeshBoolB")
    boolb.location = (1.0, 0.0, 0.0)  # A/C と x で半分重なる（重なり x[0,1]）
    return flip, dbl, sha
...
def run_calls():
    ...
    assert lo["data"]["count"] == 16, lo["data"]
    ...
    assert bi["data"]["operation"] == "INTERSECT" and bi["data"]["with"] == "MeshBoolB", bi["data"]
    ...
    # boolean の相手検証（実機でも USER_INPUT）: 自己参照 / 非mesh 相手。
    for bad in (
        {"op": "boolean", "targets": "MeshBoolB", "operation": "UNION", "with_object": "MeshBoolB"},
