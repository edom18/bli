...
def run_calls():
    ...
    assert set(lo_names) == {"Cube", "ShA", "ShB"}, lo_names  # ShA/ShB は共有mesh検証用
    assert lo["data"]["count"] == 3, lo["data"]
    ...
    # 13) delete（M6 T6.2）: 複製した独立コピーを削除 → シーンから消え、backup が返る。
    victim = created[0]
    ...
    try:
        call_retry("object-info", {"targets": victim})
        raise AssertionError("deleted object should be gone")
    except client.RpcRemoteError as e:
        assert e.error.get("message") == "E_TARGET_NOT_FOUND", e.error
    # 元 Cube は健在（delete は --targets のみに作用）
    call_retry("object-info", {"targets": "Cube"})
