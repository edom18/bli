...
### 目的
- **ゴール**: AIエージェントが Blender の一般的な操作を **CLI** で安全に自律実行できる基盤を作る。
...
### 代表ユースケース例
- ユーザはモデリング初心者であることが多く、原点・傾き・3Dプリンタ対応などの細かい調整を代行してほしい、というのが典型的な依頼（v1の3シナリオ＝原点変更・直立補正・3Dプリンタ対応はこの代表例に対応する）。
- Unity 等ゲームエンジン向けに、インポートしたモデルを調整（変形・修復・原点調整等）してから FBX/GLB で書き出す、という用途も代表例の一つ。
- これらは基盤が汎用的にカバーする操作範囲の一部であり、目的を限定するものではない。
