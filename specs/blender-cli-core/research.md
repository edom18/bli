...
### E8. print-export（STL/3MF 出力）（M8 T8.5 スパイク / 2026-06-17・5.0.1/4.4.3 **完全同値**・background 可）
研究 §[要実機検証]（`wm.stl_export` の正確な引数集合 / 3MF operator の可否）を消化した。スパイク `packages/bli-addon/spikes/print_export_spike.py`（`BLI_PRINT_EXPORT_SPIKE_BEGIN/END`）。
- **`wm.stl_export` 引数集合（5.0.1/4.4.3 同一）**: `filepath`(STR) / `ascii_format`(BOOL=False) / `export_selected_objects`(BOOL=False) / `collection`(STR) / `global_scale`(FLOAT=1.0) / `use_scene_unit`(BOOL=False) / `forward_axis`(ENUM=Y) / `up_axis`(ENUM=Z) / `apply_modifiers`(BOOL=True) / `check_existing`(BOOL=True)。旧 `export_mesh.stl` は両版とも stub（§付録C どおり）。`wm.stl_import` は `global_scale`/`use_scene_unit`/`forward_axis=Y`/`up_axis=Z`/`use_mesh_validate=True` 等。
- **background でも STL 往復が機能**: 既定 Cube（size=2）を `export_selected_objects=True` で出力 → binary 12三角形/684B（`84 + 12*50` と一致）・ascii は先頭 `solid`/1212B。再 import で頂点復元。`select_set`+`view_layer.objects.active` で対象1個に選択を絞れば background でも対象限定 export が可能。
- **`global_scale` は座標を線形倍する（決定的 golden 化可能）**: `global_scale=2.0` で export→再 import した dims が `[4,4,4]`（元 Cube=2 の2倍）。両版同値。
- **`use_scene_unit=True` は `scale_length` を掛けて出力を歪める**（`scale_length=0.001` で再 import dims がほぼ 0 に潰れた）。よって **`use_scene_unit=False` を固定**し、スケールは `global_scale` のみで支配する（spec の「`global_scale` を唯一の真実 / `scale_length` は検証専用・1000倍ずれ防止」を実機で裏付け）。`scale_length` は結果に**報告のみ**（ユーザーが 1000倍ずれ設定を検知できる検証材料）。
- **3MF は両版とも実体なし（§E6/付録C 再確認）**: `export_mesh.3mf`/`import_mesh.3mf` は stub。`addon_utils.enable("io_mesh_3mf" / "io_scene_3mf" / "bl_ext.blender_org.*")` は全て `No module named` で失敗。→ `print-export --format 3mf` は **`CAPABILITY_UNAVAILABLE`（category=ENVIRONMENT）+ STL フォールバック hint**（黙って STL に差し替えない＝要求と異なる形式を書かない）。
- 採用: `print-export --targets <name> --format stl|3mf --path <file> [--ascii] [--scale <f>]`。stl は `wm.stl_export`（`export_selected_objects=True` で対象のみ・`use_scene_unit=False`・`global_scale=<scale>`・`apply_modifiers=True`）。3mf は CAPABILITY_UNAVAILABLE。生成 STL のパス/サイズ/sha256/三角形数 + 検証用 `scale_length` を返す。

### E9. ファイルI/O（export / import 多形式）（M9 スパイク / 2026-06-17・5.0.1/4.4.3 確認・background 可）
NEXT-M9 §2.1/§2.2 を消化。スパイク `packages/bli-addon/spikes/fileio_spike.py`（`BLI_FILEIO_SPIKE_BEGIN/END`）。
- **operator 実在（両版）**: export = `wm.stl_export`/`wm.obj_export`/`export_scene.gltf`/`export_scene.fbx` 全 True。import = `wm.stl_import`/`wm.obj_import`/`import_scene.gltf` 全 True。**FBX import の唯一の版差を再確認**: 5.0=`wm.fbx_import` True+`import_scene.fbx` True / 4.4=`wm.fbx_import` **False**+`import_scene.fbx` True。→ 既存 `capability.RESOLVERS["import.fbx"] = ["wm.fbx_import", "import_scene.fbx"]` の優先順で両版正しく解決できる（5.0=新C++ / 4.4=旧Python）。3mf は両版とも `export_mesh.3mf`/`import_mesh.3mf` stub・`addon_utils.enable` 全失敗（§E8 再確認）。
- **selection 制御 param 名は形式別（最重要・形式別引数マップが必要）**: stl=`export_selected_objects`(BOOL) / obj=`export_selected_objects`(BOOL) / gltf=`use_selection`(BOOL) / fbx=`use_selection`(BOOL)。両版同一。→ OperatorResolver に**形式→selection param 名**のマップを持たせる（print-export は STL 単体だったが M9 は多形式）。
- **scale 制御 param も形式別**: stl=`global_scale`+`use_scene_unit` / obj=`global_scale`（`use_scene_unit` なし） / **gltf=scale knob なし**（`global_scale` 不在） / fbx=`global_scale`+`apply_unit_scale`(BOOL=True)+`apply_scale_options`(ENUM)。両版同一。→ 汎用 `export` は **scale=1.0 既定で素通し**（print-export が 3Dプリント用の scale 窓口）とすれば、形式ごとの scale 差異に踏み込まず最小実装で済む（gltf は scale param 自体が無いので尚更）。
- **filepath/check_existing**: 全 export に `filepath`(STR)+`check_existing`(BOOL=True 既定)。import の `check_existing` は False 既定（stl/obj）。export は `check_existing=False` を渡して既存上書き可に（print-export と同流儀）。
- **import 前後 diff で取込特定（M9 import の核）**: `before={o.name}` → import → `after=[o for o if name not in before]` で取込オブジェクトを確実に特定できた（両版）。取込名は元名と衝突すると Blender が `Cube.001` 等にリネームするため、**diff（集合差）が唯一信頼できる特定法**（名前固定では誤る）。
- **export→import 往復 bbox が全形式・両版で完全一致（往復 golden の土台）**: 既定 Cube（size=2・world AABB min[-1,-1,-1]/size[2,2,2]）を export→import した world bbox が STL/OBJ/glTF(GLB)/FBX いずれも `min[-1,-1,-1]/size[2,2,2]`。**頂点数は形式依存**（stl/obj/fbx=8 / **glTF=24**＝法線/UV シームで頂点分割）。→ **往復 golden は bbox（world AABB）を採る**（頂点数は形式間で不変でない）。
- **glTF は GLB 単一固定（v1）**: `export_scene.gltf` の `export_format` 有効値は **両版とも `('GLB', 'GLTF_SEPARATE')` のみ**（実機 enum ダンプで確認・`GLTF_EMBEDDED` は存在しない＝渡すと `TypeError: enum ... not found`）。`GLTF_SEPARATE` は `.bin`/テクスチャを別ファイルに分離し `_file_sha256_size(path)` が代表性を失うため不採用。よって **v1 は GLB 単一固定**（sha256/size がアトミック）とし、`--path` は `.glb` を必須にする（`.gltf` 等は bpy 到達前に USER_INPUT で弾き、無効 enum→INTERNAL 化を防ぐ）。GLTF_SEPARATE 対応は将来課題。
- 採用方針: 汎用 `export --format obj|fbx|gltf|stl|3mf --path <file> [--targets <name>] [--use-selection]`。`resolve_export_operator(fmt)`（RESOLVERS 委譲・§E8）→不在は CAPABILITY_UNAVAILABLE。形式別の **selection param マップ**（stl/obj=`export_selected_objects` / gltf/fbx=`use_selection`）で写像。セレクタは `--targets` 指定=その集合 / `--use-selection`=現在の選択集合 / どちらも省略=シーン全体。scale=1.0 既定素通し（print-export が scale 窓口）。glTF は GLB 単一固定（上記）。`import --format ... --path` は `resolve_import_operator` で解決し前後 diff で取込報告。3mf は import/export とも CAPABILITY。result は `{path, size, sha256, format, operator, use_selection, exported_objects}` + fingerprint=sha 先頭16桁。

### E10. save（.blend 保存）（M9 T9.3 スパイク / 2026-06-17・5.0.1/4.4.3 同値・background 可）
NEXT-M9 §2.3（save の backup）を消化。スパイク `packages/bli-addon/spikes/save_spike.py`（`BLI_SAVE_SPIKE_BEGIN/END`）。
- **`wm.save_as_mainfile` 引数集合（両版同一）**: `filepath`(STR) / `check_existing`(BOOL=True) / `compress`(BOOL=False) / `relative_remap`(BOOL=True) / `copy`(BOOL=False) ほか filter 群。`wm.save_mainfile`（現在ファイルへ保存）も実在。
- **background で save が機能**: `save_as_mainfile(filepath=...)` で .blend 生成（空シーン 92KB）。保存後 `bpy.data.filepath` が当該パスに更新される。未保存時の `bpy.data.filepath` は **空文字 `''`**（→ `--path` 省略時に空なら USER_INPUT で弾く根拠）。
- **`.blend1` backup は preferences `save_version`（既定 1）で制御**: 上書き時に Blender が既存 .blend を `<name>.blend1` へ rename（background でも機能・両版確認）。**`save_version=0` で backup 抑止 / `save_version=2` で `.blend1`+`.blend2` の多世代**（両版同値）。→ `--backup` を決定的に制御するには **`save_version` を一時上書き（`1 if backup else 0`）して try/finally で restore** する（preference 非汚染・native 機構をそのまま使うので backup naming `<name>.blend1` が Blender 標準と一致）。global preference 既定（1）に依存せず決定的。
- **`copy=True`** は current file を変えずにコピー保存（`bpy.data.filepath` 不変）。v1 の save では未使用（将来 backup 戦略の選択肢）。
- **保存 .blend の magic は版で異なる**: 4.4=非圧縮 `BLENDER-v404...` / **5.0=zstd 圧縮 `\x28\xb5\x2f\xfd...`**（`compress` RNA 既定 False でも 5.0 は zstd で書く）。どちらも valid な .blend。smoke の magic 判定は両対応（`BLENDER` or zstd magic）にする。圧縮は透過的で save の result（path/size）には影響しない（v1 は compress 制御を公開しない）。
- 採用: `save [--path <file.blend>] [--backup/--no-backup]`（mutates=True・Mode.ANY）。target = `--path`（abspath・`.blend` 必須）/ 省略時は `bpy.data.filepath`（空なら USER_INPUT）。出力先 dir 不在は USER_INPUT。`save_version` を `1 if backup else 0` に一時上書き → `run_operator(wm.save_as_mainfile, filepath=target, check_existing=False)`（message なし＝undo 不要）→ restore。result `{path(絶対), size, backed_up, backup_path}`（backed_up=backup かつ 保存前に既存・backup_path=`<target>1`）。fingerprint=metadata digest（path|size・.blend 全体 sha は大容量/非決定的のため採らない）。

### E11. open（.blend を開く・常駐サーバ生存と未保存ガード）（M9 T9.4 スパイク / 2026-06-17・5.0.1/4.4.3 同値）
NEXT-M9 §2（open の最高リスク＝常駐サーバ生存）を消化。GUI スパイク `packages/bli-addon/spikes/open_spike.py` / `open_job_spike.py`（GUI 必須＝`blender.exe --python`・`BLI_OPEN_SPIKE_*` / `BLI_OPEN_JOB_SPIKE_*`）+ is_dirty 調査 `dirty_probe_gui.py`。本番経路は **server 受信スレッド → Dispatcher.submit → pump（bpy.app.timers の `_tick` コールバック内）→ ops.dispatch → `wm.open_mainfile`** で、open は **pump タイマのコールバック内**から呼ばれる。
- **常駐サーバは open を跨いで生存する（最重要・両版実機確定）**: Dispatcher の pump タイマは `persistent=True` で登録されており（dispatcher.py: install_timer）、**`open_mainfile` 後も登録されたまま ticking を続ける**（open_spike: persistent timer 25→44 / 非 persistent timer は frozen=解除）。`bli-accept` TCP スレッド（Python スレッド・bpy 非依存）も生存。**open 後に submit→pump の往復（pong）が成立**＝dispatch は壊れない。→ **`load_post(persistent)` 等での再登録は不要**（当初の最高リスクは解消）。
- **open を含む 1 ジョブが結果を return できる（核心）**: 本番 `_open` は 1 ジョブ内で `open_mainfile` → 結果構築（scene/objects）→ return する。`open_job_spike` で **worker → submit(job: open→dict return) が dict を受け取る**ことを両版確認（filepath=開いた .blend・objects=4）。※ GUI で open を **非永続な bootstrap タイマのコールバック内**から呼ぶと後続行（print/quit）が走らない現象を観測したが、これは本番経路（persistent pump 内の 1 ジョブ）では再現しない（job 関数は open 後の行まで実行され値を返す）。
- **非 persistent timer / 非 persistent handler は file load で解除される**（ベースライン確認）。`@persistent` の `load_post` ハンドラは open で発火・生存（将来 file load フックが要るときの足がかり）。
- **`bpy.data.is_dirty` は未保存ガードに使えない（dirty_probe_gui で両版実測）**: ①background では save/open 後も **常時 True**。②GUI でも **直接 RNA 書き込み（`obj.location` 代入）では False のまま**（`ed.undo_push` や実 operator で True になる）。③致命的: GUI の dispatch/タイマ文脈では **`save_as_mainfile` 後も is_dirty が False に戻らない**（save しても True のまま）→ is_dirty ガードは「save 直後でも常に --force 要求」に退化する。→ **自前の `session_state` フラグ**で「bli が最後の save/open 以降に mutate したか」を追跡する（mutates=True コマンドの実行 **前** に modified＝pre-mark / save・open 成功で clean、dispatch で一元遷移）。**pre-mark の理由**: 途中まで mutate して例外を投げるハンドラでも open が --force を要求する安全側に倒す（実行後フックだと partial mutation を取りこぼし silent data loss・敵対的レビュー P1）。v1 は静的 `mutates` 判定で保守的（select/undo・検証失敗の mutating も modified 扱い）・bli 由来の変更のみ追跡（人間の GUI 編集/Ctrl+S は対象外）。
- 採用: `open --path <file.blend> [--force]`（mutates=True・Mode.ANY）。検証はすべて bpy 到達前（空/`.blend` 拡張子/ファイル実在=USER_INPUT・未保存ガード=`session_state` で E_PRECONDITION）。`gateway.open_blend` は **`run_operator` を使わず素の `bpy.ops.wm.open_mainfile`**（temp_override は scene 置換で teardown が壊れ得る・load は undo 境界不要）で `except Exception`→E_OPERATOR（OSError 等も）。result `{path(絶対), scene, object_count, forced, discarded_unsaved}`・fingerprint=`scene_state_fingerprint`。

### E12. render busy 検知（M10 T10.2 スパイク / 2026-06-19・5.0.1/4.4.3 同値・**GUI モード必須**）
NEXT-M10 §3-1（render handler が常駐サーバで発火するか・busy フラグの thread-safety）を消化。GUI スパイク `packages/bli-addon/spikes/render_spike.py`（`blender.exe --python`・`--background` 不可＝`bpy.app.timers` も render handler も非発火）。本番モジュール `bli_addon.render_state` を **そのまま** 使って検証した。
- **render handler は GUI 常駐サーバで発火する（両版実機確定）**: `render_init` / `render_pre` / `render_post` / `render_complete` が `bpy.ops.render.render('INVOKE_DEFAULT')`（モーダル=F12 相当）で発火。`render_init`=ジョブ開始 / `render_complete`=ジョブ終了 / `render_pre`・`render_post`=フレーム単位。busy 制御は `render_init`（busy ON）と `render_complete`＋`render_cancel`（busy OFF・キャンセル取りこぼし防止のため両方で降ろす）で行う。
- **handler は Blender の *レンダスレッド*（`Dummy-1`）から呼ばれる（メインではない・両版確定）**: モーダルレンダの handler はメインスレッドではなく内部レンダスレッドで実行される。よって busy フラグは **`threading.Event`**（GIL 下で set/clear/is_set がアトミック）で保持し、**サーバ受信スレッドが安全に `is_busy()` を読める**ようにする。スパイクの poll スレッド（受信スレッド相当）が **レンダ中に `is_busy()=True` を観測**（5.0=50・4.4=58 サンプル）＝server が dispatch 前に `BUSY_RENDERING` を即拒否できる裏付け。レンダ完了後は `is_busy()=False`。
- **`@persistent` で handler は file load（bli open）を跨いで生存する（background smoke で裏付け）**: app handler は既定で file load 時に解除されるため、`bpy.app.handlers.persistent` で `render_init`/`render_complete`/`render_cancel` をマークして登録する（付けないと `bli open` 後に busy 検知が無言で壊れる）。`smoke_ops.py` が `read_homefile` 前後で handler 生存を確認（両版 `survived_open=True`）。**注**: GUI 内で timer のコールバックから `read_homefile`/open を呼ぶと splash/再入で固まる（実測）ため、この生存確認は GUI スパイクではなく background smoke で行う。
- 採用: `render_state`（純Python の busy フラグ + bpy 依存の handler 登録を `install`/`remove` に閉じ込め）。`server._handle_rpc` が **dispatch 前**（`begin` も `settle` もする前）に「`render_busy()` かつ（`Command.mutates` または `is_heavy_request`）」なら `BUSY_RENDERING`（ENVIRONMENT・retryable）を即返す＝**キューに積まない**。read-only と lock-free（request-status）はレンダ中も通す（観測性維持）。CLI は `BUSY_RENDERING`→exit 2（TIMEOUT_PENDING 同様 retryable）。

### E13. メインスレッド応答性 watchdog（M10 T10.3 スパイク / 2026-06-20・5.0.1/4.4.3 同値・**GUI モード必須**）
NEXT-M10 §3-2（pump タイマの生存印で固まりを検知できるか・閾値の妥当性）を消化。GUI スパイク `packages/bli-addon/spikes/watchdog_spike.py`（`blender.exe --python`・`--background` 不可＝`bpy.app.timers` 非発火）。本番モジュール `bli_addon.watchdog` + `bli_addon.dispatcher` を **そのまま** 使って検証した。
- **GUI 常駐で pump タイマが生存印を更新する（両版確定）**: `dispatcher.install_timer(on_tick=watchdog.mark_alive)` が毎 tick で `last_pump_ts` を更新＝アイドル時 `snapshot().responsive=True`・`last_pump_age≈0.01s`（5.0=0.016 / 4.4=0.007）。
- **メイン占有中は pump タイマが止まり生存印が凍結する（両版確定）**: メインがイベントループへ戻らない間 `bpy.app.timers` は発火しない。実 heavy op（ico subdivisions=7 ≈1.3M 面の DECIMATE apply）で計測＝op 中に `last_pump_ts` が一度も更新されず凍結（**実 op 所要 5.0=0.247s / 4.4=0.216s** ＝1.3M 面 decimate でも 1s 未満→**閾値 60s は通常の重量 op を誤検知しない安全側**）。timer の発火可否は「メインがイベントループへ戻るか」だけで決まり、`time.sleep` でも native op でも戻らない＝凍結条件は同一。
- **占有中に別スレッド（受信スレッド相当）が unresponsive を観測できる（両版確定）**: 制御占有（`time.sleep(4s) > 閾値 1.5s`）中に poll スレッドが `watchdog.snapshot()` を高頻度サンプリングし `responsive=False` を観測（両版 90 サンプル）＝`request-status` がメイン固まり中でも「進行中だが固まっている」を返せる裏付け（lock-free）。占有解除後は pump 再開で `responsive=True` へ回復。
- **GIL 注記**: スパイクの占有は `time.sleep`（GIL 解放）で再現。実 native op が GIL を握り続ける場合、別スレッドの観測は op 中ではなく **占有解除後**に成立し得る（生存印の凍結自体はどちらも残るため検知は成立）。観測の即時性は GIL 解放の有無に依存する＝v1 の許容（spec §7 残存リスク）。
- 採用: `watchdog`（純Python＝`mark_alive`/`snapshot`/監視スレッド・bpy 非依存。生存印 `last_pump_ts` は単一 float＝GIL 下アトミックでロック不要、複合状態 `unresponsive_since` は `_lock`）。判定は **読み取り時に** `now − last_pump_ts > threshold` で計算（監視スレッド無しでも snapshot が正しい＝堅牢化）。監視スレッドは `unresponsive_since` の正確な記録 + 初回ログ通知を担う。`server._handle_rpc._request_status`（lock-free・受信スレッド処理）が応答 data に `watchdog` を載せ、CLI `doctor` も lock-free な request-status 経由で `main_thread_responsive` を取得する（ping は Dispatcher 経由で固まると応答しないため別経路）。**通知のみ＝kill しない**（重量ネイティブ op は中断不能）。閾値 `WATCHDOG_UNRESPONSIVE_THRESHOLD=60s`（DISPATCH_TIMEOUT の2倍）。

### E14. exec-python の実行セマンティクス（M11 T11.1 スパイク / 2026-06-20・5.0.1/4.4.3 **完全同値**・background 可）
NEXT-M11 §3（exec の実行セマンティクス＝bpy 注入・stdout キャプチャ・例外型）を消化。exec は新しい timer/handler 機構を要さず既存 Dispatcher のメインスレッド直列で走る＝**GUI スパイク不要**（background で十分）。スパイク `packages/bli-addon/spikes/exec_spike.py`（`blender --background --python`・両版）。スパイクの `run_user_code` がそのまま T11.1 ハンドラの実行ロジック候補。
- **(1) namespace への `bpy` 注入で bpy.data.objects を触れる（両版確定）**: `exec(compile(code,"<bli-exec>","exec"), {"bpy": bpy})` でユーザコードから `bpy.data.objects` を列挙・mutate できる（`location.x=5.0` が実際に反映＝`actual_x=5.0`）。ユーザコードが自前で `import bpy` しても可（埋め込み Python の sys.modules を共有）。
- **(2) stdout/stderr を `contextlib.redirect_stdout/redirect_stderr` でキャプチャできる（両版確定）**: `print(...)` は stdout、`print(..., file=sys.stderr)` は stderr に分離キャプチャ＝R-C の戻り値（stdout/stderr）に載せられる。
- **(3) 失敗コードの例外型は多様＝広く捕捉が必要（両版確定）**: NameError（未定義名）/ KeyError（`bpy_prop_collection` の存在しないキー）/ SyntaxError（parse/compile 失敗）を観測。いずれも `except BaseException`（ハンドラでは `except Exception`）で捕捉でき、**RuntimeError 以外も投げ得る**＝INTERNAL 化を避けるため `EXEC_ERROR`（実行時例外）へ広く写像する裏付け。SyntaxError は `ast.parse`/`compile` 段で起き、実行前に捕捉される。
- **(4) 「最終式の repr」を ast 分割で取り出せる（両版確定・REPL 流儀）**: `ast.parse(code, mode="exec")` し、`body[-1]` が `ast.Expr` なら最後の式だけ pop して `ast.Expression` に包み直し、残りを exec した後に eval して `repr(value)` を得る（`[o.name for o in ...]` の `len(names)` → `'3'`）。最後が代入文等なら `last_repr=None`。off/audited のゲートとは独立した「実行結果の観測性」を与える。**本番の精緻化**: 式の値が `None` に評価される場合は `result_repr` を `None` に抑制する（REPL の `sys.displayhook` と同じ＝`print(...)` 末尾等で `'None'` の noise を出さない）。スパイク `exec_spike.py` は観測目的で無条件 `repr` だが、本番 `exec_runner.py` はこの抑制を入れる（`0`/`""` 等の falsy は `None` ではないので抑制しない）。
- 採用: ハンドラは `exec_runner`（純Python＝ast 分割 + stdout/stderr キャプチャ + 例外写像・bpy 非依存で pytest 可。`bpy` は呼び出し側が namespace に注入）。例外は `EXEC_ERROR`（runtime=ENVIRONMENT / compile=USER_INPUT・retryable=False）へ写像し INTERNAL にしない。実行段は **`except BaseException`**＝ユーザコードの `sys.exit()`（SystemExit）/ KeyboardInterrupt がメインスレッドの dispatch を巻き込んで停止するのを防ぐ。大出力は output_ref 退避（`_ok_offload`）。実行は Dispatcher のメインスレッド直列（mutates=True）＝**job 化しない**（重量コードはメインを同期占有し得る＝M10 watchdog の通知対象・v1 制約）。
